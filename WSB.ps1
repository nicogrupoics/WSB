...
try {
    ...
    $Message.Subject = "$ComputerName Backup - Error"
    $Message.Body = $Body
...
if ($FailedBackups) {
    ...
    Write-Host ($FailedBackups, $filteredSummary | Out-String)
    ...
    $Message.To.Add($ToEmail)
    $Message.Bcc.Add($BccEmail)
    $Message.Subject = "$ComputerName Backup - Fallida"
    $Message.Body = $Body
    $Message.Priority = "High"
    ...
    Write-Host ($FailedBackups, $filteredSummary | Out-String)
    $Body = $successMsg + "`n" + ($FailedBackups, $filteredSummary | Out-String) + $footerMessage
    ...
    $Message.To.Add($ToEmail)
    $Message.Bcc.Add($BccEmail)
    $Message.Subject = "$ComputerName Backup - Correcta"
    $Message.Body = $Body
    ...
    $FileToRecover = "C:\PRUEBA RECUPERACION\ROPA LABORAL EMPLEAD@S.xlsx"  # Ruta del archivo a recuperar
